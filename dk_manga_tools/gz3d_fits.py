...
from marvin.tools.cube import Cube as CubeFast
from marvin.tools.maps import Maps
# from .DKMaps import DKMaps as Maps
...
class gz3d_fits(object):
    def __init__(self, filename, maps = None):
    ...
    def _get_spaxel_grid_xy(self, include_edges=False, grid_size=None):
        if grid_size is None:
            if self.maps is not None:
                grid_size = self.maps["emline gflux ha"].value.shape[0:]
            else:
    ...
    def get_cube(self, maps=False):
        ...
        try:
            ...
            if (maps) and (self.maps is None):
                self.maps = Maps(mangaid=manga_id)
        except:
            self.cube = 'no_data'
            if maps:
                self.maps = 'no_data'
