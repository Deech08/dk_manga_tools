...
PACKAGENAME = metadata.get('package_name', 'dk_manga_tools')
DESCRIPTION = metadata.get('description', 'Custom tools to work with SDSS MaNGA Data')
AUTHOR = metadata.get('author', 'Dhanesh (DK) Krishnarao')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/Deech08/dk_manga_tools')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.7")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: dk_manga_tools requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
